...
// ServerEvents('server.datapack.first', event => {

// 	// "Fixes" Extended Caves wiping its config contents
// 	let extcaves_conf = Java.loadClass("com.polyvalord.extcaves.config.Config")
// 	let suppl_conf = Java.loadClass("net.mehvahdjukaar.supplementaries.configs.ServerConfigs")
...
PlayerEvents.tick(event => {
	...
	// let effects = Java.loadClass("net.minecraft.potion.Effects")
...
BlockEvents.placed(event => {
	...
	if (block.getId().startsWith('pipez:energy_pipe')) {
		...
		Direction.ALL.values().forEach(face => {
			...
			sides.set(face.ordinal(), Java.loadClass("net.minecraft.nbt.ByteNBT").field_229670_c_)
