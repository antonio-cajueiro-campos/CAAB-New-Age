...
ItemEvents.tooltip(e => {
	let holds = (id, slots) => e.add("metalbarrels:" + id + "_barrel", [`§7${slots} Slots`])
	let main_assembly = (id, stage) => e.add(id, [`§7Main Assembly: ${stage == "4" ? "§6Finale" : "§6Chapter " + stage}`, '§8Consider automating this item'])
	let bonus_assembly = (id, stage) => e.add(id, [`§7Bonus Assembly: §6Chapter ${stage}`])
	let not_consumed = (id, stage) => e.add(id, [`§7Not consumed in the`, `§7Assembly Process`])
	let ore = (id, y1, y2) => e.add(id, [`§o§7Y level §6${y1} §7to §6${y2}`])

	e.add("minecraft:redstone_ore", [`§7Does not generate, crush Cinnabar to obtain Redstone.`]);
	...
	e.add("advancedrocketry:planet_id_chip", [`§3How to Use:`, `1. §7Use the Survival Mode recipes to obtain these`, `2. §7Open the UI of your Rocket`, `3. §7Click on the displayed Guidance Computer item`, `4. §7Insert chip into the empty slot`, "§8§o(Using the Planet Selection menu crashes the game)"]);
	...
	global.substrates[0].forEach(e => e.add(e.id, [`§8Category: §7Igneous`]));
	global.substrates[1].forEach(e => e.add(e.id, [`§8Category: §7Herbal`]));
	global.substrates[2].forEach(e => e.add(e.id, [`§8Category: §7Volatile`]));
	global.substrates[3].forEach(e => e.add(e.id, [`§8Category: §7Crystalline`]));
	global.substrates[4].forEach(e => e.add(e.id, [`§8Category: §7Metallurgic`]));
	global.substrates[5].forEach(e => e.add(e.id, [`§8Category: §7Gemstone`]));
	global.substrates[6].forEach(e => e.add(e.id, [`§8Category: §7Catalyst`]));

	e.add("structurescompass:structures_compass", [`§7Right-Click to Activate`]);

	e.add("magicfeather:magicfeather", [`§6Grants Creative Flight`]);

	e.add("xreliquary:alkahestry_tome", [`§6Cannot be used in Mechanical Crafting`]);

	e.add("pipez:energy_pipe", [`§7Connections may have to be`, `§7marked as §fInputs §7by sneak-clicking`, `§7the connection with a §fWrench`]);

	e.add("kubejs:accellerator_redstone", ["§7When used in Alchemy Research:", "  §6One of the §ecorrect §6Reagents",
		"  §6in §eincorrect §6slots will not be consumed"]);
		e.add("kubejs:accellerator_glowstone", ["§7When used in Alchemy Research:", "  §6One of the §ecorrect §6Reagents",
		"  §6in §ecorrect §6slots will not be consumed"]);

	for (var i = 0; i < 15; i++)
		e.add(`kubejs:failed_alchemy_${i}`, [
